...
jSmart.prototype.getTemplate = function (name) {
	...
	if (!this.templatesDir) {
		throw new Error('No templatesDir');
	}

	if (!this.globalDir) {
		throw new Error('No globalDir');
	}

	var parts = name.split(':');
	var resource = parts[0];
	var filename = parts[1];
	var filepath = path.join(this.templatesDir, filename);
	var buffer;
	...
	if (exists) {
		...
		filepath = path.join(this.globalDir, filename);
...
jSmart.prototype.registerPlugin(
	...
	function (params, data) {
		...
		var s = process(getTemplate(file, [], true), incData);
		if ('assign' in params) {
			assignVar(params.assign, s, data);
			return '';
		}
